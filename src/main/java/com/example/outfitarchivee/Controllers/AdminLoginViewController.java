    ...
    void onLoginButtonClicked(MouseEvent event) throws IOException {
        ...
        if(!PhoneNoField.getText().isEmpty()){
            ...
            else
            {
                ...
                if (dbConnection.getAdmin(phNo) == true) {

                    if(!passwordField.getText().isEmpty())
                    {
                        if(passwordField.getText().equals(Admin.getAdmin().getPassword()))
                        {
                            //play sound
                            //ts.run();
                            // set all classes

                            FXMLLoader fxmlLoader = new FXMLLoader(HelloApplication.class.getResource("FXMLFiles/admin-view.fxml"));
                            Scene scene = new Scene(fxmlLoader.load());
                            //Node node = (Node) event.getSource();
                            Stage stage = (Stage)((Node) event.getSource()).getScene().getWindow();
                            stage.setScene(scene);
